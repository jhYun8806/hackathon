...
| FAB 르탄이 캐릭터 | `Group 11.png` CSS clip-path 레이어 분리 — 몸통 고정, 머리만 까딱 애니메이션 |
| 말풍선 | FAB 위에 항상 표시되는 안내 문구, 캐릭터·버튼과 단일 래퍼로 묶여 함께 이동 |
| 답변 완료 액션 버튼 | `complete: boolean` 플래그로 스트리밍 완료 후에만 "해결됐어요 / 문의 등록하기" 노출 (슬라이드업 애니메이션) |
